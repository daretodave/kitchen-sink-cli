...
const TITLE_MATCHER = /<!--TITLE:(.*?)-->/g;
const ABOUT_MATCHER = /<!--ABOUT:(.*?)-->/g;
...
function build(store, structure, parent) {
    ...
    if(Array.isArray(structure)) {
        ...
        element.children = structure.map(block => build(store, block, parent));
...
async function emit(module, contents) {
    ...
    contents =  contents.structure.map(block => build(contents.mappings, block, module));
...
async function read(location) {
    ...
    const markdown = await fs.readFile(location, 'utf8');
    const id = path.basename(location, '.md');

    let title = TITLE_MATCHER.exec(markdown);
    if (title) {
        title = title[1];
    } else {
        title = id
    ...
    let about = ABOUT_MATCHER.exec(markdown);
    if (about) {
        about = about[1];
    } else {
        about = '';
